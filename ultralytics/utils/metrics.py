...
class ClassifyMetrics(SimpleClass, DataExportMixin):
    """
    Class for computing classification metrics including top-1, top-5 accuracy, precision, recall, f1 and average accuracy.
    """
    ...
    def results_dict(self) -> Dict[str, float]:
        ...
        return dict(zip(self.keys + ["fitness"],
                        [self.top1, self.top5, self.precision, self.recall, self.f1, self.avg_acc, self.fitness]))

    @property
    def keys(self) -> List[str]:
        """Return a list of keys for the results_dict property."""
        return ["metrics/accuracy_top1", "metrics/accuracy_top5", "metrics/precision", "metrics/recall", "metrics/f1",
                "metrics/avg_acc"]
